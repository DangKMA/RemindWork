...
const EditProject = (route) => {
    const {projectId, title, description} = route.route.params;
  ...
  const handleContinue = async () => {
    if (title.trim() === '') {
      ...
      try {
        ...
        const response = await fetch(`http://172.20.10.2:3001/api/projects/${projectId}`, {
            method: 'PUT',
            headers: {
        ...
        if (!response.ok) {
          throw new Error('Sửa dự án thất bại');
        }

        Alert.alert('Thông báo', 'Dự án đã được sửa thành công!');
