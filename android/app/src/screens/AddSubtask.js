...
const AddSubtask = ({navigation, route}) => {
  ...
  const addSubtask = async () => {
    try {
      ...
      const response = await fetch(
        `http://172.20.10.2:3001/api/tasks/${taskId}/assignees/${assigneeId}/subtasks`,
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        Alert.alert('Thành công', 'Subtask đã được thêm!');
        navigation.goBack();
      } else {
        Alert.alert('Lỗi', result.message || 'Không thể thêm subtask');
      }
      console.log("Result", result);
      
    } catch (error) {
      console.error('Error:', error);
      Alert.alert('Lỗi', 'Đã xảy ra lỗi khi thêm subtask');
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Thêm Đầu Việc</Text>
